...
export class FavouritesComponent implements OnInit {
  ...
  sortByPrice(favouriteCoin: any) {
    ...
    if (this.flagID % 2 == 1) {
      ...
      favouriteCoin.sort(function (a: { quotes: { USD: { price: any; }; }; }, b: { quotes: { USD: { price: any; }; }; }) {
        let textA = a.quotes.USD.price;
        let textB = b.quotes.USD.price;
        return (textA < textB) ? -1 : (textA > textB) ? 1 : 0;
      })
  ...
  sortByMarketCap(favouriteCoin: any) {
    this.flagID++;
    if (this.flagID % 2 == 1) {
      favouriteCoin.sort(function (a: { quotes: { USD: { market_cap: any; }; }; }, b: { quotes: { USD: { market_cap: any; }; }; }) {
